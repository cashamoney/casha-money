...
const TYPES = [
  { value: "Bank", label: "Bank Account", icon: <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.4" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M3 21h18M3 10h18M5 6l7-3 7 3M4 10v11M20 10v11M8 14v3M12 14v3M16 14v3" /></svg> },
  { value: "Cash", label: "Cash", icon: <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.4" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> },
  { value: "Credit Card", label: "Credit Card", icon: <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.4" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" /></svg> },
  { value: "UPI", label: "UPI / Digital", icon: <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.4" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg> },
  { value: "Investment", label: "Investment", icon: <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.4" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /></svg> },
  { value: "Other", label: "Other", icon: <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.4" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" /></svg> },
];
...
export default function AccountsPage() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#FAFAFA" }}>
      <div className="xw" style={{ maxWidth: 860, margin: "0 auto", padding: "40px 24px 64px" }}>
        ...
        {/* Net Worth */}
        <div style={{ background: "#fff", border: "1px solid #F0F0F0", borderRadius: 20, marginBottom: 16, overflow: "hidden" }}>
          ...
          <div style={{ padding: "28px 32px 24px" }}>
            <div className="xnw" style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 20 }}>
              <div>
                <p style={{ fontSize: 11, fontWeight: 600, color: "#71717A", margin: "0 0 6px", textTransform: "uppercase", letterSpacing: 0.08 }}>Net Worth</p>
                <p style={{ fontSize: 36, fontWeight: 800, color: "#0A0A0A", margin: 0, letterSpacing: "-0.04em", lineHeight: 1 }}>{fmt(totalBalance)}</p>
                <p style={{ fontSize: 12, color: "#A1A1AA", margin: "8px 0 0 0" }}>{accounts.length} account{accounts.length !== 1 ? "s" : ""}</p>
              </div>
              <div style={{ display: "flex", gap: 28, alignItems: "center" }}>
                <div style={{ textAlign: "right" }}>
                  <p style={{ fontSize: 10, color: "#71717A", margin: "0 0 2px", fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.05 }}>Assets</p>
                ...
                <div style={{ textAlign: "right" }}>
                  <p style={{ fontSize: 10, color: "#71717A", margin: "0 0 2px", fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.05 }}>Debts</p>
                  <p style={{ fontSize: 18, fontWeight: 800, color: debts > 0 ? "#EF4444" : "#A1A1AA", margin: 0, letterSpacing: "-0.02em" }}>{debts > 0 ? fmt(debts) : "\u2014"}</p>
                </div>
              </div>
            </div>
            {/* Bar */}
            <div style={{ marginTop: 20 }}>
              <div style={{ height: 5, borderRadius: 5, background: "#F5F5F5", overflow: "hidden" }}>
                <div style={{ width: `${assetPct}%`, height: "100%", borderRadius: 5, background: "linear-gradient(90deg, #22C55E, #16A34A)", transition: "width 0.4s" }} />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="xq" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8, marginBottom: 32 }}>
          {[
            ...
            <div key={i} style={{ background: "#fff", border: "1px solid #F0F0F0", borderRadius: 12, padding: "14px 16px", display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ width: 8, height: 8, borderRadius: 8, background: "#22C55E", flexShrink: 0 }} />
              <div>
        ...
        {grouped.length > 0 && grouped.map(g => (
          <div key={g.value} style={{ marginBottom: 24 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12, padding: "0 2px" }}>
              ...
              <span style={{ fontSize: 12, color: "#A1A1AA" }}>{g.items.length}</span>
              <div style={{ flex: 1, height: 1, background: "#F0F0F0" }} />
              <span style={{ fontSize: 14, fontWeight: 700, color: "#0A0A0A", letterSpacing: "-0.01em" }}>{fmt(g.total)}</span>
            </div>
            <div className="xcg" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(250px, 1fr))", gap: 10 }}>
              {g.items.map(a => {
                const meta = getTypeMeta(a.account_type);
                const bal = Number(a.current_balance || 0);
                const accountName = a.name || a.account_name || "Untitled";
                return (
                  <div key={a.id} style={{ background: "#fff", border: "1px solid #F0F0F0", borderRadius: 14, overflow: "hidden", transition: "border-color 0.15s, box-shadow 0.15s" }}
                  ...
                  >
                    ...
                    <div style={{ padding: "16px 18px 18px" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
                        <div style={{ width: 38, height: 38, borderRadius: 10, background: "#F0FDF4", display: "flex", alignItems: "center", justifyContent: "center", color: "#22C55E", flexShrink: 0 }}>{meta.icon}</div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <p style={{ fontSize: 13, fontWeight: 700, color: "#0A0A0A", margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{accountName}</p>
                      ...
                      <p style={{ fontSize: 9, fontWeight: 600, color: "#A1A1AA", textTransform: "uppercase", letterSpacing: 0.07, margin: "0 0 3px" }}>Balance</p>
        ...
        {accounts.length === 0 && (
          <div style={{ textAlign: "center", padding: "72px 24px 40px" }}>
            ...
            <h2 style={{ fontSize: 20, fontWeight: 800, color: "#0A0A0A", margin: "0 0 6px", letterSpacing: "-0.02em" }}>No accounts yet</h2>
            <p style={{ fontSize: 14, color: "#71717A", margin: "0 0 24px", maxWidth: 320, marginLeft: "auto", marginRight: "auto", lineHeight: 1.6 }}>Add your bank, card, or investment account to get started.</p>
            <button onClick={() => { setShowForm(true); setError(""); }}
              style={{ padding: "12px 28px", borderRadius: 12, border: "none", background: "#22C55E", color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", boxShadow: "0 4px 16px rgba(34,197,94,0.35)" }}>
      ...
      {/* Modal */}
      {showForm && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", backdropFilter: "blur(6px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999, padding: 20 }}
          ...
          <div className="xm" style={{ background: "#fff", borderRadius: 20, width: "100%", maxWidth: 440, boxShadow: "0 32px 64px rgba(0,0,0,0.18)", maxHeight: "90vh", overflowY: "auto" }}
            ...
            <div style={{ padding: "22px 26px 0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <button onClick={() => setShowForm(false)} style={{ width: 30, height: 30, borderRadius: 8, border: "1px solid #F0F0F0", background: "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "#A1A1AA" }}>
            ...
            <div style={{ padding: "18px 26px 26px" }}>
              ...
              <label style={{ display: "block", marginBottom: 8, fontSize: 10, fontWeight: 600, color: "#71717A", textTransform: "uppercase", letterSpacing: 0.06 }}>Type</label>
              <div className="xmt" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 6, marginBottom: 20 }}>
                {TYPES.map(t => {
                  const sel = type === t.value;
                  return (
                    <button key={t.value} onClick={() => setType(t.value)}
                      style={{ background: sel ? "#F0FDF4" : "#FAFAFA", border: sel ? "1.5px solid #22C55E" : "1.5px solid #F0F0F0", borderRadius: 10, padding: "12px 6px", cursor: "pointer", textAlign: "center", transition: "0.12s", fontFamily: "inherit" }}>
                      <div style={{ width: 28, height: 28, borderRadius: 7, background: sel ? "#DCFCE7" : "#F5F5F5", display: "flex", alignItems: "center", justifyContent: "center", color: sel ? "#22C55E" : "#A1A1AA", margin: "0 auto 5px" }}>{t.icon}</div>
              ...
              <div style={{ marginBottom: 14 }}>
                <label style={{ display: "block", marginBottom: 5, fontSize: 10, fontWeight: 600, color: "#71717A", textTransform: "uppercase", letterSpacing: 0.06 }}>Account Name</label>
                <input type="text" required value={name} onChange={e => setName(e.target.value)} placeholder="e.g. HDFC Savings"
                  style={{ width: "100%", height: 42, borderRadius: 10, padding: "0 14px", fontSize: 13, outline: "none", fontFamily: "inherit", background: "#FAFAFA", border: "1.5px solid #F0F0F0", color: "#0A0A0A", boxSizing: "border-box", transition: "border-color 0.15s" }}
                  onFocus={e => e.currentTarget.style.borderColor = "#22C55E"}
                  onBlur={e => e.currentTarget.style.borderColor = "#F0F0F0"} />
              </div>

              <div style={{ marginBottom: 22 }}>
                <label style={{ display: "block", marginBottom: 5, fontSize: 10, fontWeight: 600, color: "#71717A", textTransform: "uppercase", letterSpacing: 0.06 }}>Opening Balance</label>
              ...
              <button onClick={handleAdd} disabled={saving}
                style={{ width: "100%", height: 44, borderRadius: 12, border: "none", background: "#22C55E", color: "#fff", fontSize: 14, fontWeight: 700, cursor: saving ? "wait" : "pointer", fontFamily: "inherit", opacity: saving ? 0.7 : 1, boxShadow: "0 4px 16px rgba(34,197,94,0.35)" }}>
