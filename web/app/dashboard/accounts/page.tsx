...
export default function AccountsPage() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F7F7F8" }}>
      <div className="xw" style={{ maxWidth: 880, margin: "0 auto", padding: "36px 24px 64px" }}>

        {/* Breadcrumb */}
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 28 }}>
          <a href="/dashboard/overview" style={{ fontSize: 12, color: "#9CA3AF", textDecoration: "none", fontWeight: 500, transition: "color 0.12s" }}
            onMouseEnter={e => e.currentTarget.style.color = "#22C55E"}
            onMouseLeave={e => e.currentTarget.style.color = "#9CA3AF"}>Dashboard</a>
          <svg width="10" height="10" fill="none" stroke="#D1D5DB" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" /></svg>
          <span style={{ fontSize: 12, color: "#111827", fontWeight: 600 }}>Accounts</span>
        </div>

        {/* Header */}
        <div className="xh" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28, flexWrap: "wrap", gap: 16 }}>
          <h1 style={{ fontSize: 28, fontWeight: 800, color: "#111827", margin: 0, letterSpacing: "-0.04em" }}>Accounts</h1>
          <button onClick={() => { setShowForm(true); setError(""); }}
            ...
            onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-1px)"; e.currentTarget.style.boxShadow = "0 8px 20px rgba(34,197,94,0.35)"; }}
        ...
        {/* Net Worth */}
        <div style={{ background: "#fff", borderRadius: 20, marginBottom: 16, display: "flex", overflow: "hidden", border: "1px solid #EBEBEB" }}>
          <div style={{ width: 5, background: "linear-gradient(180deg, #22C55E 0%, #16A34A 100%)", flexShrink: 0 }} />
          <div style={{ flex: 1, padding: "28px 32px 24px" }}>
            <div className="xnw" style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 20 }}>
              <div>
                <p style={{ fontSize: 11, fontWeight: 600, color: "#9CA3AF", margin: "0 0 8px", textTransform: "uppercase", letterSpacing: 0.1 }}>Net Worth</p>
                <p style={{ fontSize: 38, fontWeight: 800, color: "#111827", margin: 0, letterSpacing: "-0.04em", lineHeight: 1, fontVariantNumeric: "tabular-nums" }}>{fmt(totalBalance)}</p>
                <p style={{ fontSize: 12, color: "#9CA3AF", margin: "10px 0 0 0" }}>{accounts.length} account{accounts.length !== 1 ? "s" : ""} linked</p>
              </div>
              <div style={{ display: "flex", gap: 32, alignItems: "center", paddingTop: 4 }}>
                <div style={{ textAlign: "right" }}>
                  <p style={{ fontSize: 10, color: "#9CA3AF", margin: "0 0 3px", fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.05 }}>Assets</p>
                  <p style={{ fontSize: 20, fontWeight: 800, color: "#22C55E", margin: 0, letterSpacing: "-0.02em", fontVariantNumeric: "tabular-nums" }}>{fmt(assets)}</p>
                </div>
                <div style={{ width: 1, height: 32, background: "#F0F0F0" }} />
                <div style={{ textAlign: "right" }}>
                  <p style={{ fontSize: 10, color: "#9CA3AF", margin: "0 0 3px", fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.05 }}>Debts</p>
                  <p style={{ fontSize: 20, fontWeight: 800, color: debts > 0 ? "#EF4444" : "#9CA3AF", margin: 0, letterSpacing: "-0.02em", fontVariantNumeric: "tabular-nums" }}>{debts > 0 ? fmt(debts) : "\u2014"}</p>
                </div>
              </div>
            </div>
            <div style={{ marginTop: 22 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                <span style={{ fontSize: 10, fontWeight: 600, color: "#22C55E" }}>Assets {Math.round(assetPct)}%</span>
                <span style={{ fontSize: 10, fontWeight: 600, color: debts > 0 ? "#EF4444" : "#D1D5DB" }}>Debts {Math.round(100 - assetPct)}%</span>
              </div>
              <div style={{ height: 6, borderRadius: 6, background: "#F3F4F6", overflow: "hidden" }}>
                <div style={{ width: `${assetPct}%`, height: "100%", borderRadius: 6, background: "linear-gradient(90deg, #22C55E, #16A34A)", transition: "width 0.5s ease" }} />
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Quick Stats */}
        <div className="xq" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8, marginBottom: 32 }}>
          {[
            ...
            <div key={i} style={{ background: "#fff", border: "1px solid #EBEBEB", borderRadius: 14, padding: "16px 18px", display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ width: 8, height: 8, borderRadius: 8, background: "#22C55E", flexShrink: 0, opacity: 0.6 }} />
              <div>
                <p style={{ fontSize: 18, fontWeight: 800, color: "#111827", margin: 0, lineHeight: 1, fontVariantNumeric: "tabular-nums" }}>{s.count}</p>
                <p style={{ fontSize: 10, color: "#9CA3AF", margin: "3px 0 0 0", fontWeight: 500 }}>{s.label}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        {grouped.length > 0 && grouped.map(g => (
          <div key={g.value} style={{ marginBottom: 24 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12, padding: "0 4px" }}>
              <div style={{ width: 30, height: 30, borderRadius: 8, background: "#F0FDF4", display: "flex", alignItems: "center", justifyContent: "center", color: "#22C55E", flexShrink: 0, border: "1px solid #DCFCE7" }}>{g.icon}</div>
              <span style={{ fontSize: 14, fontWeight: 700, color: "#111827" }}>{g.label}</span>
              <span style={{ fontSize: 12, color: "#9CA3AF", fontWeight: 400 }}>{g.items.length}</span>
              <div style={{ flex: 1, height: 1, background: "#F0F0F0" }} />
              <span style={{ fontSize: 14, fontWeight: 800, color: "#111827", letterSpacing: "-0.01em", fontVariantNumeric: "tabular-nums" }}>{fmt(g.total)}</span>
            </div>
            <div className="xcg" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 10 }}>
              {g.items.map(a => {
                ...
                return (
                  <div key={a.id}
                    style={{ background: "#fff", border: "1px solid #EBEBEB", borderRadius: 16, display: "flex", overflow: "hidden", transition: "border-color 0.15s, box-shadow 0.15s, transform 0.15s" }}
                    onMouseEnter={e => { e.currentTarget.style.borderColor = "#BBF7D0"; e.currentTarget.style.boxShadow = "0 8px 24px rgba(34,197,94,0.08)"; e.currentTarget.style.transform = "translateY(-2px)"; }}
                    ...
                    <div style={{ width: 4, background: "linear-gradient(180deg, #22C55E 0%, #16A34A 100%)", flexShrink: 0 }} />
                    <div style={{ flex: 1, padding: "18px 20px 20px" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
                        <div style={{ width: 40, height: 40, borderRadius: 10, background: "#F0FDF4", border: "1px solid #DCFCE7", display: "flex", alignItems: "center", justifyContent: "center", color: "#22C55E", flexShrink: 0 }}>{meta.icon}</div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <p style={{ fontSize: 14, fontWeight: 700, color: "#111827", margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{accountName}</p>
                          <p style={{ fontSize: 11, color: "#9CA3AF", margin: "2px 0 0 0" }}>{meta.label}</p>
                        </div>
                      ...
                      <p style={{ fontSize: 9, fontWeight: 600, color: "#9CA3AF", textTransform: "uppercase", letterSpacing: 0.08, margin: "0 0 4px" }}>Balance</p>
                      <p style={{ fontSize: 22, fontWeight: 800, color: bal >= 0 ? "#111827" : "#DC2626", margin: 0, letterSpacing: "-0.02em", fontVariantNumeric: "tabular-nums" }}>{fmt(bal)}</p>
                    </div>
                  </div>
                );
              })}
        ...
        {accounts.length === 0 && (
          <div style={{ textAlign: "center", padding: "80px 24px 40px" }}>
            ...
            <h2 style={{ fontSize: 20, fontWeight: 800, color: "#111827", margin: "0 0 8px", letterSpacing: "-0.02em" }}>No accounts yet</h2>
            <p style={{ fontSize: 14, color: "#6B7280", margin: "0 0 28px", maxWidth: 320, marginLeft: "auto", marginRight: "auto", lineHeight: 1.6 }}>
              Add your bank, card, or investment account to see your complete financial picture.
            </p>
            <button onClick={() => { setShowForm(true); setError(""); }}
              style={{ padding: "12px 28px", borderRadius: 12, border: "none", background: "#22C55E", color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", boxShadow: "0 4px 16px rgba(34,197,94,0.30)" }}>
      ...
      {showForm && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(17,24,39,0.45)", backdropFilter: "blur(8px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999, padding: 20 }}
          ...
          <div className="xm" style={{ background: "#fff", borderRadius: 20, width: "100%", maxWidth: 440, boxShadow: "0 32px 64px rgba(0,0,0,0.18)", maxHeight: "90vh", overflowY: "auto" }}
            onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", overflow: "hidden", borderRadius: "20px 20px 0 0" }}>
              <div style={{ width: 5, background: "linear-gradient(180deg, #22C55E, #16A34A)" }} />
              <div style={{ flex: 1, padding: "22px 24px 0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <h2 style={{ fontSize: 17, fontWeight: 800, color: "#111827", margin: 0 }}>New Account</h2>
                <button onClick={() => setShowForm(false)} style={{ width: 30, height: 30, borderRadius: 8, border: "1px solid #EBEBEB", background: "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "#9CA3AF" }}>
            ...
            <div style={{ padding: "18px 28px 28px" }}>
              ...
              <div className="xmt" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 6, marginBottom: 22 }}>
                {TYPES.map(t => {
                  ...
                  return (
                    <button key={t.value} onClick={() => setType(t.value)}
                      style={{
                        ...
                        border: sel ? "1.5px solid #22C55E" : "1.5px solid #EBEBEB",
                        borderRadius: 10, padding: "12px 6px", cursor: "pointer",
                        textAlign: "center", transition: "0.12s", fontFamily: "inherit",
                      }}>
                      <div style={{ width: 28, height: 28, borderRadius: 7, background: sel ? "#DCFCE7" : "#F3F4F6", display: "flex", alignItems: "center", justifyContent: "center", color: sel ? "#22C55E" : "#9CA3AF", margin: "0 auto 5px" }}>{t.icon}</div>
              ...
              <div style={{ marginBottom: 14 }}>
                <label style={{ display: "block", marginBottom: 6, fontSize: 10, fontWeight: 600, color: "#9CA3AF", textTransform: "uppercase", letterSpacing: 0.06 }}>Account Name</label>
                <input type="text" required value={name} onChange={e => setName(e.target.value)} placeholder="e.g. HDFC Savings"
                  style={{ width: "100%", height: 44, borderRadius: 10, padding: "0 14px", fontSize: 14, outline: "none", fontFamily: "inherit", background: "#FAFAFA", border: "1.5px solid #EBEBEB", color: "#111827", boxSizing: "border-box", transition: "border-color 0.15s" }}
                  onFocus={e => e.currentTarget.style.borderColor = "#22C55E"}
                  onBlur={e => e.currentTarget.style.borderColor = "#EBEBEB"} />
              </div>

              <div style={{ marginBottom: 24 }}>
                <label style={{ display: "block", marginBottom: 6, fontSize: 10, fontWeight: 600, color: "#9CA3AF", textTransform: "uppercase", letterSpacing: 0.06 }}>Opening Balance</label>
              ...
              <button onClick={handleAdd} disabled={saving}
                ...
                onMouseEnter={e => e.currentTarget.style.boxShadow = "0 8px 24px rgba(34,197,94,0.35)"}
                onMouseLeave={e => e.currentTarget.style.boxShadow = "0 4px 16px rgba(34,197,94,0.30)"}>
