...
export default function AccountsPage() {
  ...
  const handleAdd = async () => {
    if (!name.trim() || !balance) return; setSaving(true);
    const { data: u } = await supabase.auth.getUser();
    if (!u?.user) return;
  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto" }}>
      ...
      {showForm && (
        <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 14, padding: 24, marginBottom: 20 }}>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. HDFC Savings" style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: "1px solid var(--border)", background: "var(--bg)", color: "var(--text)", fontSize: 13, outline: "none" }} />
          </div>
          <div style={{ marginBottom: 16 }}>
            <label style={{ fontSize: 11, fontWeight: 600, color: "var(--muted)", display: "block", marginBottom: 8 }}>Account Type</label>
            <div className="acc-type-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8 }}>
              {TYPES.map(t => {
                const selected = type === t.value;
                return (
                  <button key={t.value} onClick={() => setType(t.value)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 12px", borderRadius: 10, border: selected ? `2px solid ${t.color}` : "2px solid var(--border)", background: selected ? `${t.color}0D` : "var(--bg)", cursor: "pointer", transition: "0.15s", textAlign: "left" }}>
                    <div style={{ width: 32, height: 32, borderRadius: 8, background: selected ? `${t.color}1A` : "var(--panel-alt)", display: "flex", alignItems: "center", justifyContent: "center", color: selected ? t.color : "var(--muted)", flexShrink: 0 }}>{t.icon}</div>
                    <span style={{ fontSize: 12, fontWeight: selected ? 700 : 500, color: selected ? t.color : "var(--text)" }}>{t.label}</span>
                  </button>
                );
              })}
            </div>
          ...
          <div style={{ display: "flex", gap: 12, alignItems: "flex-end", flexWrap: "wrap" }}>
            <div style={{ flex: 1, minWidth: 160 }}>
              ...
              <input type="number" value={balance} onChange={e => setBalance(e.target.value)} placeholder="0" style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: "1px solid var(--border)", background: "var(--bg)", color: "var(--text)", fontSize: 13, outline: "none" }} />
            </div>
      ...
      {accounts.length === 0 ? (
        <p style={{ fontSize: 13, color: "var(--faint)", margin: "12px 0 0 2px" }}>No accounts yet. Add your first one above.</p>
      ) : (
        <div className="acc-card-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 16 }}>
          {accounts.map(a => {
            const meta = getTypeMeta(a.account_type);
            return (
              <div key={a.id} style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 14, padding: 20, transition: "0.15s" }} className="acc-card">
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
                  <div style={{ width: 42, height: 42, borderRadius: 12, background: `${meta.color}14`, display: "flex", alignItems: "center", justifyContent: "center", color: meta.color, flexShrink: 0 }}>{meta.icon}</div>
      ...
      <style>{`
        ...
        @media (max-width: 640px) {
          .acc-type-grid { grid-template-columns: repeat(2, 1fr) !important; }
