      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr", gap: 16, marginBottom: 20 }}>
        ...
        <div style={{
          ...
          <div style={{ position: "relative", zIndex: 2 }}>
            ...
            <p style={{ fontSize: 13, color: "rgba(255,255,255,0.55)", margin: "0 0 20px", lineHeight: 1.5 }}>
              Your finances are <span style={{ color: healthColor, fontWeight: 700 }}>{healthLabel}</span>. {hs >= 700 ? "Keep building consistently." : "Focus on reducing debt and increasing savings."}
            </p>
            {/* Component bars */}
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {[
                { label: "Cash Buffer", value: cashScore, color: "#22C55E" },
                { label: "Debt Load", value: debtScore, color: "#3B82F6" },
                { label: "Savings Rate", value: savingsScore, color: "#06B6D4" },
                { label: "Goal Progress", value: goalScore, color: "#8B5CF6" },
              ].map((b, i) => (
                <div key={i}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                    <span style={{ fontSize: 11, color: "rgba(255,255,255,0.6)" }}>{b.label}</span>
                    <span style={{ fontSize: 11, color: "rgba(255,255,255,0.35)" }}>{Math.round(b.value)}%</span>
                  </div>
                  <div style={{ height: 5, borderRadius: 9, background: "rgba(255,255,255,0.08)" }}>
          ...
          <div style={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
            <div style={{
              ...
              height: 130,
              borderRadius: "50%",
              background: `conic-gradient(${healthColor} 0deg ${(hs / 1000) * 360}deg, rgba(255,255,255,0.06) ${(hs / 1000) * 360}deg 360deg)`,
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              flexShrink: 0,
            }}>
              <div style={{ width: 96, height: 96, borderRadius: "50%", background: "#111113", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                <span style={{ fontSize: 18, fontWeight: 800, color: "#fff", lineHeight: 1 }}>{healthLabel}</span>
                <span style={{ fontSize: 9, color: "rgba(255,255,255,0.3)", marginTop: 4 }}>score status</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <Link href="/dashboard/accounts" style={{ textDecoration: "none" }}>
            <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 14, padding: 20, cursor: "pointer" }}>
              <p style={{ fontSize: 10, fontWeight: 700, color: "var(--faint)", textTransform: "uppercase", letterSpacing: 0.06, margin: "0 0 6px" }}>Net Worth</p>
              <p style={{ fontSize: 24, fontWeight: 800, margin: 0, letterSpacing: "-0.02em", color: "var(--text)" }}>{fmt(nw)}</p>
              <p style={{ fontSize: 11, color: "var(--muted)", margin: "4px 0 0 0" }}>Assets {fmt(bal)} · Debt {fmt(debtTotal)}</p>
            </div>
          </Link>
          <Link href="/dashboard/transactions" style={{ textDecoration: "none" }}>
            <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 14, padding: 20, cursor: "pointer" }}>
              <p style={{ fontSize: 10, fontWeight: 700, color: "var(--faint)", textTransform: "uppercase", letterSpacing: 0.06, margin: "0 0 6px" }}>Monthly Income</p>
              <p style={{ fontSize: 22, fontWeight: 800, color: "#22C55E", margin: 0 }}>{fmt(inc)}</p>
              <p style={{ fontSize: 11, color: "var(--muted)", margin: "3px 0 0 0" }}>{sr}% savings rate</p>
            </div>
          </Link>
          <Link href="/dashboard/transactions" style={{ textDecoration: "none" }}>
            <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 14, padding: 20, cursor: "pointer" }}>
              <p style={{ fontSize: 10, fontWeight: 700, color: "var(--faint)", textTransform: "uppercase", letterSpacing: 0.06, margin: "0 0 6px" }}>Monthly Spent</p>
              <p style={{ fontSize: 22, fontWeight: 800, margin: 0, color: "var(--text)" }}>{fmt(exp)}</p>
              <p style={{ fontSize: 11, color: remaining >= 0 ? "var(--green-text)" : "#DC2626", margin: "3px 0 0 0" }}>{remaining >= 0 ? `${fmt(remaining)} remaining` : "Over budget"}</p>
            </div>
          </Link>
        </div>
      </div>
        ...
        <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 14, padding: 20 }}>
          ...
          <ResponsiveContainer width="100%" height={210}>
            <AreaChart data={series}>
              <defs>
                <linearGradient id="gI" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#22C55E" stopOpacity={0.15} /><stop offset="95%" stopColor="#22C55E" stopOpacity={0} /></linearGradient>
                <linearGradient id="gE" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#94A3B8" stopOpacity={0.1} /><stop offset="95%" stopColor="#94A3B8" stopOpacity={0} /></linearGradient>
              </defs>
              ...
              <Area type="monotone" dataKey="expense" stroke="var(--faint)" strokeWidth={1.5} fill="url(#gE)" strokeDasharray="4 4" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 14, padding: 20 }}>
          ...
          {pie.length === 0 ? <div style={{ height: 210, display: "flex", alignItems: "center", justifyContent: "center", color: "var(--faint)", fontSize: 12 }}>No expense data yet</div> : (
            <div>
              <ResponsiveContainer width="100%" height={150}>
                <PieChart><Pie data={pie} innerRadius={45} outerRadius={70} paddingAngle={3} dataKey="value">{pie.map((_, i) => <Cell key={i} fill={clr[i % clr.length]} stroke="none" />)}</Pie><Tooltip /></PieChart>
              </ResponsiveContainer>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 16 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <Link href="/dashboard/goals" style={{ textDecoration: "none" }}>
            <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 14, padding: 18, cursor: "pointer" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                <h3 style={{ fontSize: 13, fontWeight: 700, margin: 0, color: "var(--text)" }}>Active Goals</h3>
              ...
              {goals.length === 0 ? (
                <p style={{ fontSize: 12, color: "var(--faint)", margin: 0 }}>No active goals yet. Set one to start tracking.</p>
              ) : (
                goals.slice(0, 3).map((g, i) => {
                  const p = Number(g.target_amount) > 0 ? (Number(g.current_amount || 0) / Number(g.target_amount)) * 100 : 0;
                  return (
                    <div key={i} style={{ marginBottom: i < Math.min(goals.length, 3) - 1 ? 10 : 0 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 4 }}>
                        <span style={{ fontWeight: 600, color: "var(--text)" }}>{g.name}</span>
                        <span style={{ color: "var(--faint)" }}>{p.toFixed(0)}%</span>
                      </div>
                      <div style={{ height: 5, borderRadius: 9, background: "var(--panel-alt)" }}>
                        <div style={{ width: `${Math.min(p, 100)}%`, height: "100%", borderRadius: 9, background: p >= 100 ? "#22C55E" : "#3B82F6" }} />
                      </div>
                    </div>
                  );
                })
          ...
          <Link href="/dashboard/debts" style={{ textDecoration: "none" }}>
            <div style={{ background: debtTotal > 0 ? "var(--red-soft)" : "var(--card)", border: `1px solid ${debtTotal > 0 ? "var(--red-border)" : "var(--border)"}`, borderRadius: 14, padding: 18, cursor: "pointer" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                <p style={{ fontSize: 10, fontWeight: 700, color: debtTotal > 0 ? "var(--red-text)" : "var(--faint)", textTransform: "uppercase", margin: 0 }}>Total Debt</p>
                <svg width="14" height="14" fill="none" stroke={debtTotal > 0 ? "var(--red-text)" : "var(--faint)"} strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" /></svg>
              </div>
              <p style={{ fontSize: 22, fontWeight: 800, color: debtTotal > 0 ? "#DC2626" : "var(--text)", margin: 0 }}>{fmt(debtTotal)}</p>
              <p style={{ fontSize: 11, color: debtTotal > 0 ? "var(--red-text)" : "var(--muted)", margin: "4px 0 0 0" }}>
                {debtTotal > 0 ? `${debts.length} active loan${debts.length > 1 ? "s" : ""}` : "You are debt free!"}
              </p>
            </div>
          </Link>
          ...
          <div style={{ background: "var(--green-soft)", border: "1px solid var(--green-border)", borderRadius: 14, padding: 18 }}>
            ...
            <p style={{ fontSize: 11, color: "var(--green-text)", margin: "4px 0 0 0" }}>
              {sr >= 30 ? "Excellent! You're saving well." : sr >= 10 ? "Good start. Aim for 30%." : "Try to increase your savings."}
            </p>
          </div>
        </div>
      </div>
