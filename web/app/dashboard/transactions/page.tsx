...
function autoDetect(accounts: Account[]): Transaction[] {
  ...
  for (var i = 0; i < 8 + Math.floor(Math.random() * 12); i++) {
    ...
    var inc = ["Salary", "Freelance", "Investment", "Refund", "Bonus", "Gift", "Rental", "Interest", "Dividend"].indexOf(m.c) >= 0;
    var amt = m.lo + Math.round((Math.random() * (m.hi - m.lo)) * 100) / 100;
    var ago = Math.floor(Math.random() * 30);
    var dd = new Date(now.getTime() - ago * 86400000);
    var acc = accounts[Math.floor(Math.random() * accounts.length)];
    r.push({ id: uid(), amount: inc ? amt : -amt, type: inc ? "income" : "expense", merchant: m.n, category: m.c, date: ds(dd), note: "", source: "auto" as "manual" | "sms" | "csv" | "auto", accountId: acc.id });
  }
...
function pLine(l: string, defId: string): PE | null {
  ...
  var merchant = "";
  if (isIncome && fromM) merchant = fromM[1];
  else if (!isIncome && toM) merchant = toM[1];
  else if (fromM) merchant = fromM[1];
  else if (toM) merchant = toM[1];
  else if (dashM) merchant = dashM[1];
...
function pText(text: string, accounts: Account[]): PE[] {
  var d = accounts.length > 0 ? accounts[0].id : "";
...
function DetailContent(props: { tx: Transaction; accounts: Account[]; onClose: () => void; onDelete: (id: string) => void; onEdit: (t: Transaction) => void }) {
  ...
  var tf = isTf(tx);
  var tfP: Account | null = null;
  var tfD: "out" | "in" = "out";
  if (tx.merchant.startsWith("Transfer to ")) {
    tfP = props.accounts.find(function (a) { return a.name === tx.merchant.replace("Transfer to ", ""); }) || null;
    tfD = "out";
  } else if (tx.merchant.startsWith("Transfer from ")) {
    tfP = props.accounts.find(function (a) { return a.name === tx.merchant.replace("Transfer from ", ""); }) || null;
    tfD = "in";
  }

  var startEd = function () {
    setEm(tx.merchant); setEa(String(Math.abs(tx.amount))); setEt(tx.type); setEc(tx.category); setEdt(tx.date); setEn(tx.note); setEai(tx.accountId || ""); setEd(true);
  };
  var saveEd = function () {
    var amt = parseFloat(ea);
    if (!amt || amt <= 0) return;
    props.onEdit({ ...tx, merchant: em.trim() || tx.merchant, amount: et === "expense" ? -amt : amt, type: et, category: ec, date: edt, note: en, accountId: eai || undefined });
    setEd(false);
  };
  ...
  if (ed) {
    return (
      <div>
        ...
        <select value={ec} onChange={function (e) { setEc(e.target.value); }} style={{ width: "100%", height: 32, padding: "0 8px", borderRadius: 7, background: "var(--surface)", border: "1px solid var(--border)", color: "var(--text)", fontSize: 12, outline: "none", fontFamily: "inherit", marginBottom: 6, cursor: "pointer" }}>{cats.map(function (c2) { return <option key={c2} value={c2}>{c2}</option>; })}</select>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          <button onClick={function () { setEd(false); }} style={{ flex: 1, height: 36, borderRadius: 8, background: "transparent", border: "1px solid var(--border)", color: "var(--muted)", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>Cancel</button>
          <button onClick={saveEd} disabled={!em.trim() || !ea} style={{ flex: 1, height: 36, borderRadius: 8, background: em.trim() && ea ? "var(--green)" : "var(--card)", border: "none", color: em.trim() && ea ? "#fff" : "var(--faint)", fontSize: 12, fontWeight: 600, cursor: em.trim() && ea ? "pointer" : "not-allowed", fontFamily: "inherit" }}>Save</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      {acc && <div style={{ padding: "10px 12px", borderRadius: 8, background: "var(--surface)", border: "1px solid var(--border)", marginBottom: 8 }}>
        <p style={{ fontSize: 9, fontWeight: 600, color: "var(--muted)", textTransform: "uppercase", margin: "0 0 6px 0" }}>Account</p>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{ width: 32, height: 32, borderRadius: 8, background: "linear-gradient(135deg, " + ag(acc.type)[0] + ", " + ag(acc.type)[1] + ")", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><span style={{ fontSize: 10, fontWeight: 800, color: "#fff" }}>{ai(acc.type)}</span></div>
          <div><p style={{ fontSize: 13, fontWeight: 600, color: "var(--text)", margin: 0 }}>{acc.name}</p><p style={{ fontSize: 10, color: "var(--muted)", margin: "1px 0 0 0" }}>{acc.type}</p></div>
        </div>
      </div>}
      {tf && tfP && <div style={{ padding: "10px 12px", borderRadius: 8, background: "var(--surface)", border: "1px solid var(--border)", marginBottom: 8 }}>
        <p style={{ fontSize: 9, fontWeight: 600, color: "var(--muted)", textTransform: "uppercase", margin: "0 0 6px 0" }}>{tfD === "out" ? "To" : "From"}</p>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{ width: 32, height: 32, borderRadius: 8, background: "linear-gradient(135deg, " + ag(tfP.type)[0] + ", " + ag(tfP.type)[1] + ")", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><span style={{ fontSize: 10, fontWeight: 800, color: "#fff" }}>{ai(tfP.type)}</span></div>
          <div><p style={{ fontSize: 13, fontWeight: 600, color: "var(--text)", margin: 0 }}>{tfP.name}</p></div>
        </div>
      </div>}
      ...
      <div style={{ display: "flex", gap: 6 }}>
        ...
        <button onClick={startEd} style={{ height: 36, padding: "0 14px", borderRadius: 8, background: "transparent", border: "1px solid var(--green-border)", color: "var(--green)", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>Edit</button>
        <button onClick={function () { props.onDelete(tx.id); }} style={{ height: 36, padding: "0 14px", borderRadius: 8, background: "transparent", border: "1px solid var(--red-border)", color: "var(--red)", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>Delete</button>
      </div>
    </div>
  );
}
...
export default function TransactionsPage() {
  ...
  var showToast = function (m: string) { setToast(m); setTimeout(function () { setToast(""); }, 2500); };
  var getAcc = function (id: string | undefined) { if (!id) return null; return accounts.find(function (a) { return a.id === id; }) || null; };
  var getTfP = function (t: Transaction) {
  ...
  var gDR = function () {
    ...
    else if (datePreset === "week") { from = ds(new Date(now.getTime() - now.getDay() * 86400000)); }
  ...
  var doDetect = function () {
    setDetecting(true);
    setTimeout(function () { var d = autoDetect(accounts); setTransactions(function (p) { return d.concat(p); }); setDetecting(false); setShowDetect(false); showToast(d.length + " detected"); }, 2000);
  };
  ...
  var doImport = function () {
    ...
    var nw: Transaction[] = sel.map(function (e) {
      return { id: uid(), amount: e.isIncome ? e.amount : -e.amount, type: (e.isIncome ? "income" : "expense") as "income" | "expense", merchant: e.merchant, category: e.category, date: e.date, note: "", source: importMode as "manual" | "sms" | "csv" | "auto", accountId: e.accountId || undefined };
    });
    setTransactions(function (p) { return nw.concat(p); });
    setParsed([]); setStmtText(""); setShowImport(false); showToast(nw.length + " imported");
  };
  var doAdd = function () {
    ...
    setAddForm({ merchant: "", amount: "", type: "expense", category: "Other", date: ds(new Date()), note: "", accountId: "" });
    setShowAdd(false); showToast(tx.merchant + " added");
  };
  var doDelete = function (id: string) { setTransactions(function (p) { return p.filter(function (t) { return t.id !== id; }); }); showToast("Deleted"); setShowDetail(false); setDetailTx(null); };
  var doEdit = function (u: Transaction) { setTransactions(function (p) { return p.map(function (t) { return t.id === u.id ? u : t; }); }); showToast("Updated"); setShowDetail(false); setDetailTx(null); };
  ...
  var dms = dim(calY, calM);
  var fds = fday(calY, calM);
  var calCells: (number | null)[] = [];
  for (var ci2 = 0; ci2 < fds; ci2++) calCells.push(null);
  for (var di2 = 1; di2 <= dms; di2++) calCells.push(di2);
  var calWeeks: (number | null)[][] = [];
  ...
  return (
    <div style={{ maxWidth: 960, margin: "0 auto", padding: "28px 0 40px" }} onClick={closeAll}>
      {toast && <div style={{ position: "fixed", top: 24, left: "50%", transform: "translateX(-50%)", zIndex: 100, background: "var(--green)", color: "#fff", padding: "10px 24px", borderRadius: 10, fontSize: 13, fontWeight: 600, fontFamily: "inherit", boxShadow: "0 4px 20px rgba(26,143,78,0.3)" }}>{toast}</div>}
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 8, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <div style={{ position: "relative" }} onClick={sp}>
          ...
          {oSort && <div style={getDropStyle(oSort)}>
            {(["newest", "oldest", "highest", "lowest", "merchant"] as SortOption[]).map(function (s) {
              return <button key={s} onClick={function (e) { e.stopPropagation(); setSort(s); setOSort(false); }} style={{ display: "flex", alignItems: "center", gap: 6, width: "100%", textAlign: "left", padding: "7px 10px", borderRadius: 6, border: "none", background: sort === s ? "var(--green-dim)" : "transparent", color: sort === s ? "var(--green)" : "var(--text)", fontSize: 11, fontWeight: sort === s ? 700 : 500, cursor: "pointer", fontFamily: "inherit" }}>{sort === s && <span style={{ width: 5, height: 5, borderRadius: 3, background: "var(--green)", flexShrink: 0 }} />}{SL[s]}</button>;
            })}
          </div>}
        </div>
        ...
        <div style={{ position: "relative" }} onClick={sp}>
          ...
          {oCat && <div style={{ ...getDropStyle(oCat), minWidth: 240, padding: 8 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 3 }}>
              {ALL_CATS.map(function (c2) {
                var info = ci(c2);
                var on2 = filterCat === c2;
                return <button key={c2} onClick={function (e) { e.stopPropagation(); setFilterCat(c2); setOCat(false); }} style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 7px", borderRadius: 6, border: "1px solid " + (on2 ? info.color + "50" : "transparent"), background: on2 ? info.bg : "transparent", color: on2 ? info.color : "var(--text)", fontSize: 9, fontWeight: on2 ? 700 : 500, cursor: "pointer", fontFamily: "inherit", whiteSpace: "nowrap" }}><span style={{ width: 18, height: 18, borderRadius: 5, background: "linear-gradient(135deg, " + info.color + "20, " + info.color + "40)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 6, fontWeight: 800, color: info.color, flexShrink: 0 }}>{info.label}</span>{c2}</button>;
              })}
            </div>
          </div>}
        </div>
        ...
        <div style={{ position: "relative" }} onClick={sp}>
          ...
          {oDate && <div style={{ ...getDropStyle(oDate), right: 0, left: "auto", width: 280, padding: 10 }}>
            <div style={{ display: "flex", gap: 4, flexWrap: "wrap", marginBottom: 8 }}>
              {(["today", "yesterday", "week", "month", "30days"] as DatePreset[]).map(function (p) {
                return <button key={p} onClick={function (e) { e.stopPropagation(); setDatePreset(p); setODate(false); }} style={{ padding: "4px 8px", borderRadius: 5, border: "1px solid " + (datePreset === p ? "var(--green-border)" : "var(--border)"), background: datePreset === p ? "var(--green-dim)" : "transparent", color: datePreset === p ? "var(--green)" : "var(--muted)", fontSize: 9, fontWeight: datePreset === p ? 700 : 500, cursor: "pointer", fontFamily: "inherit" }}>{DL[p]}</button>;
              })}
              <button onClick={function (e) { e.stopPropagation(); setDatePreset("all"); setODate(false); }} style={{ padding: "4px 8px", borderRadius: 5, border: "1px solid " + (datePreset === "all" ? "var(--green-border)" : "var(--border)"), background: datePreset === "all" ? "var(--green-dim)" : "transparent", color: datePreset === "all" ? "var(--green)" : "var(--muted)", fontSize: 9, fontWeight: datePreset === "all" ? 700 : 500, cursor: "pointer", fontFamily: "inherit" }}>All Time</button>
            </div>
            <div style={{ borderTop: "1px solid var(--border)", paddingTop: 8 }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                <button onClick={function (e) { e.stopPropagation(); if (calM === 0) { setCalM(11); setCalY(calY - 1); } else { setCalM(calM - 1); } }} style={{ width: 28, height: 28, borderRadius: 7, border: "1px solid var(--border)", background: "var(--surface)", color: "var(--text)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontFamily: "inherit" }}>{"<"}</button>
                <span style={{ fontSize: 12, fontWeight: 700, color: "var(--text)", letterSpacing: 0.3 }}>{MOS[calM]} {calY}</span>
                <button onClick={function (e) { e.stopPropagation(); if (calM === 11) { setCalM(0); setCalY(calY + 1); } else { setCalM(calM + 1); } }} style={{ width: 28, height: 28, borderRadius: 7, border: "1px solid var(--border)", background: "var(--surface)", color: "var(--text)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontFamily: "inherit" }}>{">"}</button>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 2, marginBottom: 2 }}>
                {["S", "M", "T", "W", "T", "F", "S"].map(function (d3, i3) { return <div key={i3} style={{ textAlign: "center", fontSize: 8, fontWeight: 700, color: "var(--muted)", padding: "2px 0" }}>{d3}</div>; })}
              </div>
              {calWeeks.map(function (week, wi3) {
                return <div key={wi3} style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 2 }}>
                  {week.map(function (day, di3) {
                    if (day === null) return <div key={di3} style={{ height: 30 }} />;
                    var dateStr = calY + "-" + String(calM + 1).padStart(2, "0") + "-" + String(day).padStart(2, "0");
                    var isT = dateStr === today;
                    var isF = dateStr === calFrom;
                    var isTo2 = dateStr === calTo;
                    var inR = calFrom && calTo && dateStr > calFrom && dateStr < calTo;
                    var bg = isF || isTo2 ? "var(--green)" : inR ? "var(--green-dim)" : isT ? "var(--green-dim)" : "transparent";
                    var col = isF || isTo2 ? "#fff" : inR || isT ? "var(--green)" : "var(--text)";
                    return <button key={di3} onClick={function (e) { e.stopPropagation(); onCalDay(dateStr); }} style={{ height: 30, borderRadius: 6, border: isT && !isF && !isTo2 ? "1.5px solid var(--green)" : "none", background: bg, color: col, fontSize: 10, fontWeight: isF || isTo2 || isT ? 700 : 400, cursor: "pointer", fontFamily: "inherit" }}>{day}</button>;
                  })}
                </div>;
              })}
              {calFrom && <div style={{ marginTop: 8, paddingTop: 8, borderTop: "1px solid var(--border)" }}>
                ...
                <button onClick={function (e) { e.stopPropagation(); applyCal(); }} style={{ width: "100%", height: 28, borderRadius: 6, background: "var(--green)", border: "none", color: "#fff", fontSize: 10, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>Apply Custom Range</button>
              </div>}
            </div>
          </div>}
        </div>
        ...
        {accounts.length > 0 && <div style={{ position: "relative" }} onClick={sp}>
          ...
          {oAcc && <div style={{ ...getDropStyle(oAcc), minWidth: 180 }}>
            ...
            {accounts.map(function (a) {
              var on3 = filterAcc === a.id;
              return <button key={a.id} onClick={function (e) { e.stopPropagation(); setFilterAcc(a.id); setOAcc(false); }} style={{ display: "flex", alignItems: "center", gap: 6, width: "100%", textAlign: "left", padding: "7px 10px", borderRadius: 6, border: "none", background: on3 ? "var(--green-dim)" : "transparent", color: on3 ? "var(--green)" : "var(--text)", fontSize: 11, fontWeight: on3 ? 700 : 500, cursor: "pointer", fontFamily: "inherit" }}><div style={{ width: 20, height: 20, borderRadius: 5, background: "linear-gradient(135deg, " + ag(a.type)[0] + ", " + ag(a.type)[1] + ")", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><span style={{ fontSize: 7, fontWeight: 800, color: "#fff" }}>{ai(a.type)}</span></div>{a.name}</button>;
            })}
          </div>}
        </div>}
      </div>
      ...
      <div style={{ marginBottom: 12 }}>
        <input type="text" placeholder="Search merchant, category, or note..." value={q} onChange={function (e) { setQ(e.target.value); setVCount(30); }} style={{ width: "100%", height: 36, padding: "0 12px", borderRadius: 8, background: "var(--surface)", border: "1px solid var(--border)", color: "var(--text)", fontSize: 12, outline: "none", fontFamily: "inherit" }} />
      </div>
            ...
            return <div key={g.date}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                {g.items.map(function (tx) {
                  var c3 = ci(tx.category);
                  var acc2 = getAcc(tx.accountId);
                  var tf2 = isTf(tx);
                  var tfi2 = tf2 ? getTfP(tx) : null;
                  return <div key={tx.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", borderRadius: 8, background: "var(--surface)", border: "1px solid var(--border)", cursor: "pointer", transition: "all 150ms ease" }} onClick={function (e) { e.stopPropagation(); setDetailTx(tx); setShowDetail(true); }}>
                    <div style={{ width: 36, height: 36, borderRadius: 8, background: c3.bg, border: "1px solid " + c3.color + "25", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><span style={{ fontSize: 10, fontWeight: 800, color: c3.color }}>{c3.label}</span></div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                        ...
                        {tf2 && <span style={{ fontSize: 7, fontWeight: 700, color: "#8B5CF6", background: "#FAF5FF", padding: "1px 4px", borderRadius: 2, textTransform: "uppercase", flexShrink: 0 }}>TRANSFER</span>}
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 2 }}>
                        {acc2 && <div style={{ display: "flex", alignItems: "center", gap: 3 }}><div style={{ width: 16, height: 16, borderRadius: 4, background: "linear-gradient(135deg, " + ag(acc2.type)[0] + ", " + ag(acc2.type)[1] + ")", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><span style={{ fontSize: 6, fontWeight: 800, color: "#fff" }}>{ai(acc2.type)}</span></div><span style={{ fontSize: 10, color: "var(--muted)", fontWeight: 500 }}>{acc2.name}</span></div>}
                        {tf2 && tfi2 && tfi2.p && <div style={{ display: "flex", alignItems: "center", gap: 3 }}><svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="var(--muted)" strokeWidth="2.5"><path d="M5 12h14" /><path d="M12 5l7 7-7 7" /></svg><div style={{ width: 16, height: 16, borderRadius: 4, background: "linear-gradient(135deg, " + ag(tfi2.p.type)[0] + ", " + ag(tfi2.p.type)[1] + ")", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><span style={{ fontSize: 6, fontWeight: 800, color: "#fff" }}>{ai(tfi2.p.type)}</span></div><span style={{ fontSize: 10, color: tfi2.d === "out" ? "var(--red)" : "var(--green)", fontWeight: 500 }}>{tfi2.d === "out" ? "to" : "from"} {tfi2.p.name}</span></div>}
                        {!acc2 && !tf2 && <span style={{ fontSize: 10, color: "var(--muted)" }}>{tx.category}</span>}
                      </div>
                    </div>
      ...
      {showDetail && detailTx && <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.5)", backdropFilter: "blur(12px)" }} onClick={function () { setShowDetail(false); setDetailTx(null); }}><div style={{ background: "var(--bg)", borderRadius: 16, padding: 22, width: "100%", maxWidth: 380, boxShadow: "0 20px 60px rgba(0,0,0,0.3)", border: "1px solid var(--border)" }} onClick={function (e) { e.stopPropagation(); }}><DetailContent tx={detailTx} accounts={accounts} onClose={function () { setShowDetail(false); setDetailTx(null); }} onDelete={doDelete} onEdit={doEdit} /></div></div>}

      {showAdd && <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.5)", backdropFilter: "blur(12px)" }} onClick={function () { setShowAdd(false); }}><div style={{ background: "var(--bg)", borderRadius: 16, padding: 22, width: "100%", maxWidth: 380, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.3)", border: "1px solid var(--border)" }} onClick={function (e) { e.stopPropagation(); }}>
        ...
        <select value={addForm.category} onChange={function (e) { setAddForm(function (f) { return { ...f, category: e.target.value }; }); }} style={{ width: "100%", height: 36, padding: "0 10px", borderRadius: 8, background: "var(--surface)", border: "1px solid var(--border)", color: "var(--text)", fontSize: 12, outline: "none", fontFamily: "inherit", marginBottom: 6, cursor: "pointer" }}>{addCats.map(function (c4) { return <option key={c4} value={c4}>{c4}</option>; })}</select>
        ...
        {accounts.length > 0 && <select value={addForm.accountId} onChange={function (e) { setAddForm(function (f) { return { ...f, accountId: e.target.value }; }); }} style={{ width: "100%", height: 36, padding: "0 10px", borderRadius: 8, background: "var(--surface)", border: "1px solid var(--border)", color: "var(--text)", fontSize: 12, outline: "none", fontFamily: "inherit", marginBottom: 6, cursor: "pointer" }}><option value="">No account</option>{accounts.map(function (a2) { return <option key={a2.id} value={a2.id}>{a2.name} ({a2.type})</option>; })}</select>}
        ...
        <div style={{ display: "flex", gap: 6 }}>
          ...
          <button onClick={doAdd} disabled={!addForm.merchant.trim() || !addForm.amount} style={{ flex: 1, height: 38, borderRadius: 8, background: addForm.merchant.trim() && addForm.amount ? "var(--green)" : "var(--card)", border: "none", color: addForm.merchant.trim() && addForm.amount ? "#fff" : "var(--faint)", fontSize: 12, fontWeight: 600, cursor: addForm.merchant.trim() && addForm.amount ? "pointer" : "not-allowed", fontFamily: "inherit" }}>Add</button>
        </div>
      </div></div>}

      {showDetect && <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.5)", backdropFilter: "blur(12px)" }} onClick={function () { if (!detecting) setShowDetect(false); }}><div style={{ background: "var(--bg)", borderRadius: 16, padding: 22, width: "100%", maxWidth: 380, boxShadow: "0 20px 60px rgba(0,0,0,0.3)", border: "1px solid var(--border)", textAlign: "center" }} onClick={function (e) { e.stopPropagation(); }}>
        ...
        {!detecting && accounts.length > 0 && <p style={{ fontSize: 10, color: "var(--muted)", margin: "0 0 16px 0" }}>{accounts.length} account{accounts.length > 1 ? "s" : ""}: {accounts.map(function (a3) { return a3.name; }).join(", ")}</p>}
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={function (e) { e.stopPropagation(); if (!detecting) setShowDetect(false); }} disabled={detecting} style={{ flex: 1, height: 40, borderRadius: 8, background: "transparent", border: "1px solid var(--border)", color: detecting ? "var(--faint)" : "var(--muted)", fontSize: 12, fontWeight: 600, cursor: detecting ? "not-allowed" : "pointer", fontFamily: "inherit" }}>Cancel</button>
          <button onClick={function (e) { e.stopPropagation(); doDetect(); }} disabled={detecting || accounts.length === 0} style={{ flex: 1, height: 40, borderRadius: 8, background: detecting || accounts.length === 0 ? "var(--card)" : "linear-gradient(135deg, #1A8F4E, #2DD4BF)", border: "none", color: detecting || accounts.length === 0 ? "var(--faint)" : "#fff", fontSize: 12, fontWeight: 600, cursor: detecting || accounts.length === 0 ? "not-allowed" : "pointer", fontFamily: "inherit" }}>{detecting ? "Scanning..." : "Detect Now"}</button>
        </div>
      </div></div>}

      {showImport && <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.5)", backdropFilter: "blur(12px)" }} onClick={function () { setShowImport(false); }}><div style={{ background: "var(--bg)", borderRadius: 16, padding: 22, width: "100%", maxWidth: 400, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.3)", border: "1px solid var(--border)" }} onClick={function (e) { e.stopPropagation(); }}>
        <h2 style={{ fontSize: 16, fontWeight: 700, color: "var(--text)", margin: "0 0 4px 0" }}>Import Transactions</h2>
        <p style={{ fontSize: 11, color: "var(--muted)", margin: "0 0 12px 0" }}>Paste SMS or CSV. We auto-detect amounts, dates and merchants.</p>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4, marginBottom: 10 }}>
          <button onClick={function () { setImportMode("sms"); setParsed([]); }} style={{ height: 36, borderRadius: 8, border: "1px solid " + (importMode === "sms" ? "#F97316" : "var(--border)"), background: importMode === "sms" ? "#FFF7ED" : "transparent", color: importMode === "sms" ? "#F97316" : "var(--muted)", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", justifyContent: "center", gap: 5 }}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z" /></svg>SMS</button>
        ...
        <textarea value={stmtText} onChange={function (e) { setStmtText(e.target.value); setParsed([]); }} placeholder={importMode === "sms" ? "Your Rs.250.00 txn from A/C XX1234 to Swiggy on 15/01/25 was debited." : "15/01/2025,Swiggy,250.00,expense"} style={{ width: "100%", height: 90, borderRadius: 8, padding: "10px", fontSize: 11, fontFamily: "monospace", background: "var(--bg)", border: "1px solid var(--border)", color: "var(--text)", outline: "none", resize: "none", lineHeight: 1.5, marginBottom: 8 }} />
        <button onClick={doParse} disabled={!stmtText.trim()} style={{ height: 34, padding: "0 14px", borderRadius: 8, background: stmtText.trim() ? (importMode === "sms" ? "#F97316" : "#3B82F6") : "var(--card)", border: "none", color: stmtText.trim() ? "#fff" : "var(--faint)", fontSize: 11, fontWeight: 600, cursor: stmtText.trim() ? "pointer" : "not-allowed", fontFamily: "inherit", marginBottom: 8, width: "100%" }}>{importMode === "sms" ? "Parse SMS" : "Parse CSV"}</button>
        {parsed.length > 0 && <div style={{ marginTop: 8 }}>
          <p style={{ fontSize: 11, fontWeight: 700, color: "var(--text)", margin: "0 0 6px 0" }}>Found {parsed.length} - toggle to select</p>
          <div style={{ display: "flex", flexDirection: "column", gap: 3, marginBottom: 8, maxHeight: 200, overflowY: "auto" }}>
            {parsed.map(function (en, idx) {
              var c4 = ci(en.category);
              return <div key={idx} onClick={function (e) { e.stopPropagation(); togP(idx); }} style={{ display: "flex", alignItems: "center", gap: 6, padding: "5px 8px", borderRadius: 5, background: en.selected ? "var(--green-dim)" : "var(--bg)", border: "1px solid " + (en.selected ? "var(--green-border)" : "var(--border)"), cursor: "pointer", opacity: en.selected ? 1 : 0.4 }}>
                <div style={{ width: 14, height: 14, borderRadius: 3, border: en.selected ? "none" : "2px solid var(--border)", background: en.selected ? "var(--green)" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{en.selected && <svg width="7" height="7" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="3"><polyline points="20 6 9 17 4 12" /></svg>}</div>
                <span style={{ width: 16, height: 11, borderRadius: 2, background: c4.bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 5, fontWeight: 800, color: c4.color, flexShrink: 0 }}>{c4.label}</span>
                <span style={{ flex: 1, fontSize: 10, fontWeight: 600, color: "var(--text)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{en.merchant}</span>
                <span style={{ fontSize: 10, fontWeight: 700, color: en.isIncome ? "var(--green)" : "var(--red)", fontVariantNumeric: "tabular-nums", flexShrink: 0 }}>{en.isIncome ? "+" : "-"}{fm(en.amount)}</span>
              </div>;
            })}
          </div>
          <button onClick={function (e) { e.stopPropagation(); doImport(); }} style={{ height: 34, padding: "0 14px", borderRadius: 8, background: "var(--green)", border: "none", color: "#fff", fontSize: 11, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", width: "100%" }}>Add {parsed.filter(function (e2) { return e2.selected; }).length} Transactions</button>
        </div>}
