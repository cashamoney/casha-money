...
const CATEGORIES: Record<string, { color: string; icon: React.ReactNode }> = {
  ...
  "Investment Return": { color: "#15803D", icon: <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v2m0 8v2" /></svg> },
  ...
  "Refund": { color: "#86EFAC", icon: <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 15L3 9m0 0l6-6M3 9h12a6 6 0 010 12h-3" /></svg> },
  ...
  "Subscription": { color: "#7C3AED", icon: <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg> },
...
function groupTxns(txns: any[]) {
  ...
  txns.forEach(t => { const d = new Date(t.transaction_date); if (isToday(d)) g["Today"].push(t); else if (isYesterday(d)) g["Yesterday"].push(t); else if (isThisWeek(d)) g["This Week"].push(t); else g["Earlier"].push(t); });
...
export default function TransactionsPage() {
  ...
  const filtered = useMemo(() => {
    return txns.filter(t => {
      ...
      if (search) { const s = search.toLowerCase(); if (!((t.merchant_name || "").toLowerCase().includes(s) || (t.category || "").toLowerCase().includes(s))) return false; }
      return true;
    });
  ...
  const handleAdd = async () => {
    if (!fMerchant.trim() || !fAmount) return; setSaving(true);
  ...
  const handleDelete = async (id: string) => { if (!confirm("Delete this transaction?")) return; await supabase.from("transactions").delete().eq("id", id); load(); };
  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto" }}>
      ...
      <div className="resp-grid-3" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 20 }}>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 16, flexWrap: "wrap", alignItems: "center" }}>
        <div style={{ flex: 1, minWidth: 180, position: "relative" }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search..." style={{ width: "100%", padding: "9px 12px 9px 34px", borderRadius: 10, border: "1px solid var(--border)", background: "var(--card)", color: "var(--text)", fontSize: 13, outline: "none" }} />
        </div>
        <div style={{ display: "flex", gap: 4 }}>
          {["all", "income", "expense"].map(f => (
            <button key={f} onClick={() => setTypeFilter(f)} style={{ padding: "8px 14px", borderRadius: 8, border: "1px solid", borderColor: typeFilter === f ? "#22C55E" : "var(--border)", background: typeFilter === f ? "rgba(34,197,94,0.1)" : "var(--card)", color: typeFilter === f ? "#22C55E" : "var(--muted)", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
              {f === "all" ? "All" : f === "income" ? "Income" : "Expense"}
            </button>
          ))}
        </div>
        <div ref={catRef} style={{ position: "relative" }}>
          <button onClick={() => setCatOpen(v => !v)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 14px", borderRadius: 8, border: "1px solid var(--border)", background: "var(--card)", color: catFilter !== "all" ? getCatMeta(catFilter).color : "var(--muted)", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
            {selectedCatMeta ? (<><div style={{ width: 20, height: 20, borderRadius: 5, background: `${selectedCatMeta.color}14`, display: "flex", alignItems: "center", justifyContent: "center", color: selectedCatMeta.color }}>{selectedCatMeta.icon}</div><span>{catFilter}</span></>) : (<><svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M4 6h16M4 10h16M4 14h16M4 18h16" /></svg><span>All Categories</span></>)}
          ...
          {catOpen && (
            <div style={{ position: "absolute", top: 42, right: 0, minWidth: 220, maxHeight: 320, overflowY: "auto", background: "var(--card)", border: "1px solid var(--border)", borderRadius: 12, boxShadow: "0 10px 30px rgba(0,0,0,0.12)", zIndex: 20, padding: 6 }}>
              <button onClick={() => { setCatFilter("all"); setCatOpen(false); }} style={{ width: "100%", display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", borderRadius: 8, border: "none", background: catFilter === "all" ? "var(--panel-alt)" : "transparent", color: "var(--text)", fontSize: 12, fontWeight: catFilter === "all" ? 600 : 500, cursor: "pointer", textAlign: "left" }}>
                <div style={{ width: 22, height: 22, borderRadius: 6, background: "var(--panel-alt)", display: "flex", alignItems: "center", justifyContent: "center" }}><svg width="12" height="12" fill="none" stroke="var(--muted)" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M4 6h16M4 10h16M4 14h16M4 18h16" /></svg></div>
                All Categories
              </button>
              {allCats.map(c => { const m = getCatMeta(c); const sel = catFilter === c; return (
                <button key={c} onClick={() => { setCatFilter(c); setCatOpen(false); }} style={{ width: "100%", display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", borderRadius: 8, border: "none", background: sel ? `${m.color}0D` : "transparent", color: sel ? m.color : "var(--text)", fontSize: 12, fontWeight: sel ? 600 : 500, cursor: "pointer", textAlign: "left" }}>
                  <div style={{ width: 22, height: 22, borderRadius: 6, background: `${m.color}14`, display: "flex", alignItems: "center", justifyContent: "center", color: m.color }}>{m.icon}</div>{c}
                </button>
              ); })}
            </div>
          )}
      ...
      {(showAdd || editId) && (
        <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 14, padding: 20, marginBottom: 20 }}>
          ...
          <div className="resp-grid-2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
            <div><label style={{ fontSize: 11, fontWeight: 600, color: "var(--muted)", display: "block", marginBottom: 6 }}>Merchant / Description</label><input value={fMerchant} onChange={e => setFMerchant(e.target.value)} placeholder="e.g. Swiggy" style={{ width: "100%", padding: "9px 12px", borderRadius: 8, border: "1px solid var(--border)", background: "var(--bg)", color: "var(--text)", fontSize: 13, outline: "none" }} /></div>
            <div><label style={{ fontSize: 11, fontWeight: 600, color: "var(--muted)", display: "block", marginBottom: 6 }}>Amount</label><input type="number" value={fAmount} onChange={e => setFAmount(e.target.value)} placeholder="0" style={{ width: "100%", padding: "9px 12px", borderRadius: 8, border: "1px solid var(--border)", background: "var(--bg)", color: "var(--text)", fontSize: 13, outline: "none" }} /></div>
          </div>
          <div style={{ marginBottom: 16 }}><label style={{ fontSize: 11, fontWeight: 600, color: "var(--muted)", display: "block", marginBottom: 6 }}>Date</label><input type="date" value={fDate} onChange={e => setFDate(e.target.value)} style={{ width: "100%", padding: "9px 12px", borderRadius: 8, border: "1px solid var(--border)", background: "var(--bg)", color: "var(--text)", fontSize: 13, outline: "none" }} /></div>
          <div style={{ marginBottom: 16 }}>
            <label style={{ fontSize: 11, fontWeight: 600, color: "var(--muted)", display: "block", marginBottom: 8 }}>Category</label>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              {activeCats.map(c => { const m = getCatMeta(c); const sel = fCat === c; return (
                <button key={c} onClick={() => setFCat(c)} style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 10px", borderRadius: 8, border: "1px solid", borderColor: sel ? m.color : "var(--border)", background: sel ? `${m.color}0D` : "var(--bg)", cursor: "pointer" }}>
      ...
      {filtered.length === 0 ? (
        <p style={{ fontSize: 13, color: "var(--faint)", margin: "12px 0 0 2px" }}>{search || typeFilter !== "all" || catFilter !== "all" ? "No transactions match your filters." : "No transactions yet. Add your first one above."}</p>
      ) : (
        <>
          {groups.map(([label, items]) => (
            <div key={label} style={{ marginBottom: 20 }}>
              ...
              <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 14, overflow: "hidden" }}>
                {items.map((t, i) => (
                  <div key={t.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 16px", borderBottom: i < items.length - 1 ? "1px solid var(--tx-border)" : "none" }} className="txn-row">
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
                      ...
                      <div style={{ display: "flex", gap: 2 }} className="txn-actions">
                        <button onClick={() => startEdit(t)} style={{ width: 28, height: 28, borderRadius: 6, background: "var(--panel-alt)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--muted)" }}><svg width="12" height="12" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg></button>
                        <button onClick={() => handleDelete(t.id)} style={{ width: 28, height: 28, borderRadius: 6, background: "rgba(220,38,38,0.06)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "#DC2626" }}><svg width="12" height="12" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          {visibleCount < filtered.length && (
            <div style={{ textAlign: "center", marginTop: 8 }}><button onClick={() => setVisibleCount(v => v + 20)} style={{ padding: "8px 20px", borderRadius: 10, background: "var(--card)", border: "1px solid var(--border)", color: "var(--text)", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Load More</button></div>
          )}
        </>
      )}
