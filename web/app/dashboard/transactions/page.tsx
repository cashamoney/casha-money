...
var CATEGORIES = [
  ...
  { name: "Income", color: "#22C55E", bg: "#F0FDF4", letters: "IN" },
...
function smartParse(text: string): { amount: number; merchant: string; category: string; date: string; isIncome: boolean } | null {
  ...
  else if (l.includes("uber") || l.includes("ola") || l.includes("fuel") || l.includes("petrol") || l.includes("lyft") || l.includes("gas") || l.includes("metro") || l.includes("train") || l.includes("cab") || l.includes("taxi")) cat = "Transport";
  else if (l.includes("netflix") || l.includes("hotstar") || l.includes("spotify") || l.includes("hulu") || l.includes("movie") || l.includes("game") || l.includes("gaming")) cat = "Entertainment";
  else if (l.includes("amazon") || l.includes("flipkart") || l.includes("myntra") || l.includes("target") || l.includes("walmart") || l.includes("shop") || l.includes("store") || l.includes("mall")) cat = "Shopping";
  else if (l.includes("rent") || l.includes("housing") || l.includes("lease")) cat = "Rent";
  else if (l.includes("electricity") || l.includes("bill") || l.includes("water") || l.includes("utility") || l.includes("internet") || l.includes("phone") || l.includes("recharge")) cat = "Bills";
  else if (l.includes("salary") || l.includes("credited") || l.includes("income") || l.includes("received") || l.includes("deposited") || l.includes("refund") || l.includes("dividend") || l.includes("interest")) cat = "Income";
  ...
  else if (l.includes("course") || l.includes("school") || l.includes("college") || l.includes("tuition") || l.includes("book") || l.includes("udemy") || l.includes("coursera")) cat = "Education";
  else if (l.includes("stock") || l.includes("mutual fund") || l.includes("sip") || l.includes("fd") || l.includes("invest")) cat = "Investment";
  var isIncome = l.includes("credited") || l.includes("received") || l.includes("deposited") || l.includes("salary") || l.includes("income") || l.includes("refund") || l.includes("dividend") || l.includes("interest") || l.includes("cashback");
  ...
  return { amount: amount, merchant: merchant, category: isIncome ? "Income" : cat, date: date, isIncome: isIncome };
}
...
function CategoryIcon(props: { name: string; size?: number }) {
  ...
  return (
    <div style={{ width: s, height: s, borderRadius: Math.round(s * 0.28), background: cat.bg, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
      <span style={{ fontSize: Math.round(s * 0.3), fontWeight: 800, color: cat.color, letterSpacing: -0.5 }}>{cat.letters}</span>
    </div>
  );
}
      ...
      >
        <span style={{ display: "flex", alignItems: "center", gap: 6 }}>
          {selected && selected.letters && (
            <span style={{ width: 18, height: 18, borderRadius: 4, background: selected.color ? selected.color + "15" : "var(--surface)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 8, fontWeight: 800, color: selected.color || "var(--text)" }}>{selected.letters}</span>
          )}
        ...
        <div style={{
          ...
          {props.options.map(function (opt) {
            ...
            return (
              ...
              >
                {opt.letters && (
                  <span style={{ width: 20, height: 20, borderRadius: 5, background: opt.color ? opt.color + "15" : "var(--surface)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 8, fontWeight: 800, color: opt.color || "var(--muted)" }}>{opt.letters}</span>
                )}
...
function Calendar(props: { value: string; onChange: (val: string) => void }) {
  var [open, setOpen] = useState(false);
  var ref = useRef<HTMLDivElement>(null);
  ...
  return (
    <div ref={ref} style={{ position: "relative" }}>
      <button onClick={function () { setOpen(function (p) { return !p; }); }}
        style={{ width: "100%", height: 46, padding: "0 14px", borderRadius: 10, background: "var(--surface)", border: "1px solid " + (open ? "var(--green-border)" : "var(--border)"), color: "var(--text)", fontSize: 14, outline: "none", fontFamily: "inherit", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between", transition: "all 200ms ease", boxShadow: open ? "0 0 0 3px var(--green-dim)" : "none" }}>
      ...
      {open && (
        <div style={{ position: "absolute", top: 54, left: 0, zIndex: 30, background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 14, padding: 16, boxShadow: "var(--shadow-lg)", animation: "fadeIn 150ms cubic-bezier(0.16, 1, 0.3, 1)", width: 280 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 2 }}>
            {days.map(function (day, idx) {
              if (day === null) return <div key={"e" + idx} style={{ height: 32 }} />;
              var m = String(viewMonth + 1).padStart(2, "0");
              var dd = String(day).padStart(2, "0");
              ...
              return (
                <button key={day} onClick={function () { selectDay(day); }}
                  style={{ height: 32, borderRadius: 8, border: "none", cursor: "pointer", fontFamily: "inherit", fontSize: 12, fontWeight: isSel ? 700 : 500, color: isSel ? "#fff" : "var(--text)", background: isSel ? "var(--green)" : isToday ? "var(--green-dim)" : "transparent", transition: "all 120ms ease", display: "flex", alignItems: "center", justifyContent: "center" }}
...
export default function TransactionsPage() {
  ...
  var addManual = function () {
    ...
    var t: Transaction = { id: generateId(), amount: addForm.type === "expense" ? -amt : amt, type: addForm.type, merchant: addForm.merchant.trim(), category: addForm.type === "income" ? "Income" : addForm.category, date: addForm.date, note: addForm.note, source: "manual" };
  ...
  var addSms = function () {
    ...
    setSmsError("");
    var t: Transaction = { id: generateId(), amount: parsed.isIncome ? parsed.amount : -parsed.amount, type: parsed.isIncome ? "income" : "expense", merchant: parsed.merchant, category: parsed.isIncome ? "Income" : parsed.category, date: parsed.date, note: "", source: "sms" };
    setTransactions(function (prev) { return [t, ...prev]; });
    setSmsText("");
    setShowSms(false);
  };
  ...
  var catOptions = [{ value: "All", label: "All categories", letters: "AC", color: "#6B7280" }].concat(
    CATEGORIES.map(function (c) { return { value: c.name, label: c.name, letters: c.letters, color: c.color }; })
  );
    ...
    <div style={{ maxWidth: 960, margin: "0 auto", padding: "32px 0" }}>
      ...
      {showAdd && (
        <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.4)", backdropFilter: "blur(8px)", animation: "fadeIn 200ms ease" }} onClick={function () { setShowAdd(false); }}>
          <div style={{ background: "var(--bg)", borderRadius: 20, padding: 32, width: "100%", maxWidth: 460, boxShadow: "var(--shadow-xl)", border: "1px solid var(--border)", animation: "fadeIn 250ms cubic-bezier(0.16, 1, 0.3, 1)", maxHeight: "90vh", overflowY: "auto" }} onClick={function (e) { e.stopPropagation(); }}>
            <h2 style={{ fontSize: 20, fontWeight: 700, color: "var(--text)", margin: "0 0 20px 0" }}>Add Transaction</h2>

            {/* Type toggle */}
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 18, background: "var(--surface)", borderRadius: 12, padding: 4 }}>
              {(["expense", "income"] as const).map(function (tp) {
                var isActive = addForm.type === tp;
                return (
                  <button key={tp} onClick={function () { setAddForm(function (f) { return { ...f, type: tp, category: tp === "income" ? "Income" : f.category }; }); }}
                    style={{ padding: "10px 0", borderRadius: 10, border: "none", background: isActive ? "var(--bg)" : "transparent", color: isActive ? "var(--text)" : "var(--muted)", fontSize: 13, fontWeight: isActive ? 700 : 500, cursor: "pointer", fontFamily: "inherit", transition: "all 200ms cubic-bezier(0.16, 1, 0.3, 1)", boxShadow: isActive ? "var(--shadow-sm)" : "none", transform: isActive ? "scale(1.02)" : "scale(1)" }}>
            ...
            <input type="text" placeholder="Merchant / Description" value={addForm.merchant} onChange={function (e) { setAddForm(function (f) { return { ...f, merchant: e.target.value }; }); }}
              style={{ width: "100%", height: 46, padding: "0 14px", borderRadius: 10, background: "var(--surface)", border: "1px solid var(--border)", color: "var(--text)", fontSize: 14, outline: "none", fontFamily: "inherit", marginBottom: 14, transition: "all 200ms ease" }}
              onFocus={function (e) { e.currentTarget.style.borderColor = "var(--green-border)"; e.currentTarget.style.boxShadow = "0 0 0 3px var(--green-dim)"; }}
              onBlur={function (e) { e.currentTarget.style.borderColor = "var(--border)"; e.currentTarget.style.boxShadow = "none"; }} />

            {/* Category buttons */}
            <p style={{ fontSize: 11, fontWeight: 600, color: "var(--muted)", marginBottom: 8, textTransform: "uppercase", letterSpacing: 0.06 }}>Category</p>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 14 }}>
              {(addForm.type === "income" ? CATEGORIES : CATEGORIES.filter(function (c) { return c.name !== "Income"; })).map(function (c) {
                var isActive = addForm.category === c.name;
                return (
                  <button key={c.name} onClick={function () { setAddForm(function (f) { return { ...f, category: c.name }; }); }}
                    style={{ padding: "7px 12px", borderRadius: 8, border: "1px solid " + (isActive ? c.color + "40" : "var(--border)"), background: isActive ? c.bg : "transparent", cursor: "pointer", fontFamily: "inherit", transition: "all 150ms ease", display: "flex", alignItems: "center", gap: 5, transform: isActive ? "scale(1.05)" : "scale(1)" }}>
                    ...
                    <span style={{ fontSize: 12, fontWeight: isActive ? 700 : 500, color: isActive ? c.color : "var(--muted)" }}>{c.name}</span>
                  </button>
                );
              })}
            </div>
            ...
            {/* Calendar */}
            <p style={{ fontSize: 11, fontWeight: 600, color: "var(--muted)", marginBottom: 8, textTransform: "uppercase", letterSpacing: 0.06 }}>Date</p>
            <div style={{ marginBottom: 14 }}>
              <Calendar value={addForm.date} onChange={function (val) { setAddForm(function (f) { return { ...f, date: val }; }); }} />
            </div>
      ...
      {showSms && (
        <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.4)", backdropFilter: "blur(8px)", animation: "fadeIn 200ms ease" }} onClick={function () { setShowSms(false); }}>
          <div style={{ background: "var(--bg)", borderRadius: 20, padding: 32, width: "100%", maxWidth: 480, boxShadow: "var(--shadow-xl)", border: "1px solid var(--border)", animation: "fadeIn 250ms cubic-bezier(0.16, 1, 0.3, 1)" }} onClick={function (e) { e.stopPropagation(); }}>
            ...
            <textarea value={smsText} onChange={function (e) { setSmsText(e.target.value); setSmsError(""); }} placeholder={"Rs.2,500 debited from A/c XX1234. Info: Swiggy\n\n$45.00 spent at Starbucks\n\nINR 50000 credited to your account - Salary"}
              style={{ width: "100%", height: 130, borderRadius: 12, padding: "14px", fontSize: 13, fontFamily: "inherit", background: "var(--surface)", border: "1px solid " + (smsError ? "var(--red-border)" : "var(--border)"), color: "var(--text)", outline: "none", resize: "none", lineHeight: 1.6, marginBottom: smsError ? 10 : 12, transition: "all 200ms ease" }}
            ...
            {smsText.trim() && smartParse(smsText) && !smsError && (
              <div style={{ background: "var(--green-dim)", border: "1px solid var(--green-border)", borderRadius: 12, padding: "14px 16px", marginBottom: 16, animation: "fadeIn 200ms ease" }}>
                <p style={{ fontSize: 11, fontWeight: 600, color: "var(--green)", margin: "0 0 10px 0", textTransform: "uppercase", letterSpacing: 0.06 }}>Detected</p>
                  ...
                  {[
                    ...
                    return (
                      <div key={r.label} style={{ display: "flex", justifyContent: "space-between" }}>
                        ...
                        <span style={{ fontSize: 12, fontWeight: 600, color: r.color, fontVariantNumeric: "tabular-nums" }}>{r.value}</span>
                      </div>
                    );
                  })}
                </div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={function () { setShowSms(false); }} style={{ flex: 1, height: 46, borderRadius: 10, background: "transparent", border: "1px solid var(--border)", color: "var(--muted)", fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", transition: "all 150ms ease" }}
                onMouseEnter={function (e) { e.currentTarget.style.background = "var(--surface)"; }}
                onMouseLeave={function (e) { e.currentTarget.style.background = "transparent"; }}>Cancel</button>
              <button onClick={addSms} style={{ flex: 1, height: 46, borderRadius: 10, background: "var(--green)", border: "none", color: "#fff", fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", transition: "all 200ms ease", boxShadow: "0 2px 8px rgba(26, 143, 78, 0.2)" }}
                onMouseEnter={function (e) { e.currentTarget.style.background = "var(--green-soft)"; e.currentTarget.style.transform = "translateY(-1px)"; }}
